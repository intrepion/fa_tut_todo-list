...
func TestPostgresTaskStoreCreatesTasksWithIncreasingIds(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`INSERT INTO tasks (text) VALUES ($1) RETURNING id, text`)).
		...
		WillReturnRows(sqlmock.NewRows([]string{"id", "text"}).AddRow(int64(1), "Learn how to invert binary trees"))
	mock.ExpectQuery(regexp.QuoteMeta(`INSERT INTO tasks (text) VALUES ($1) RETURNING id, text`)).
		WithArgs("Buy milk").
		WillReturnRows(sqlmock.NewRows([]string{"id", "text"}).AddRow(int64(2), "Buy milk"))
